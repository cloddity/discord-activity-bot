...
async def on_message(message):
    ...
    vent = get_channel(client.get_all_channels(), 'archive')
    ...
    if message.content.startswith('!stat'):
        ...
        ymdA = datetime(int(array[1]), int(array[2]), int(array[3]) - 1)
        ymdB = datetime(int(array[1]), int(array[2]), int(array[3]) + 1)
        mesg = await client.send_message(general_channel, 'Calculating...')
        counter = 0
        async for msg in client.logs_from(vent, limit=10000, before=ymdB):
            if msg.author == message.author:
                counter += 1
        #async for msg in client.logs_from(516507986147934215, limit=100, before=ymdB, after=ymdA):
            #if msg.author == message.author:
                #counter += 1
        await client.edit_message(mesg, '{} has {} messages in {}.'.format(message.author, str(counter), vent))
